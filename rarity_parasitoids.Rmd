...
par_fig_table_BC <-merge(Bray_C_figure_par, distance_plot, by=c("Locality_A", "Locality_B")) %>% 
  filter (hodnota > 0)
par_fig_table_BC$dataset <-"All plants"
# 
...
par_fig_table_BC_rare <-merge(Bray_C_figure_par_rare, distance_plot, by=c("Locality_A", "Locality_B")) %>% 
  filter (hodnota > 0)
par_fig_table_BC_rare$dataset <- "All plants"
# 
...
par_fig_table_BC_common <-merge(Bray_C_figure_par_common, distance_plot, by=c("Locality_A", "Locality_B")) %>% 
  filter (hodnota > 0)
par_fig_table_BC_common$dataset <-"All plants"
# 
...
par_fig_table_CS <-merge(Chao_Sor_figure_par, distance_plot, by=c("Locality_A", "Locality_B")) %>% 
  filter (hodnota > 0)
par_fig_table_CS$Dataset <-"All plants"
...
para_pivot2_rare[is.na(para_pivot2_rare)] <- 0 # Replacing NA values with 0
# Chao-Sorensen index for parasitoids
CS_par_res_rare<- dis.chao(para_pivot2_rare, index="sorensen", version="rare", freq=NULL) %>%  as.matrix()
...
Chao_Sor_figure_par_rare <- dplyr::rename (Chao_Sor_parasitoids_rare, Locality_A = Var1, Locality_B = Var2, hodnota = value)
...
par_fig_table_CS_rare <-merge(Chao_Sor_figure_par_rare, distance_plot, by=c("Locality_A", "Locality_B")) %>% 
  filter (hodnota > 0)
par_fig_table_CS_rare$Dataset <-"All plants"
...
para_pivot2_common[is.na(para_pivot2_common)] <- 0 # Replacing NA values with 0
# Chao-Sorensen index for parasitoids
CS_par_res_common<- dis.chao(para_pivot2_common, index="sorensen", version="rare", freq=NULL) %>%  as.matrix()
...
par_fig_table_CS_common <-merge(Chao_Sor_figure_par_common, distance_plot, by=c("Locality_A", "Locality_B")) %>% 
  filter (hodnota > 0)
par_fig_table_CS_common$Dataset <-"All plants"
...
par_fig_table_sor <-merge(Sorensen_figure, distance_plot, by=c("Locality_A", "Locality_B")) %>% 
  filter (hodnota > 0)
par_fig_table_sor$dataset <-"All plants"
...
par_fig_table_sor_rare  <-merge(Sorensen_figure_rare , distance_plot, by=c("Locality_A", "Locality_B")) %>% 
  filter (hodnota > 0)
par_fig_table_sor_rare$dataset <-"All plants"
...
par_fig_table_sor_common  <-merge(Sorensen_figure_common , distance_plot, by=c("Locality_A", "Locality_B")) %>% 
  filter (hodnota > 0)
par_fig_table_sor_common$dataset <-"All plants"
...
```
# Common - Bray-Curtis - ALL
```{r}
#Bray-Curtis index for parasitoids
BC_par_res_common_plants <- vegdist(para_pivot2_common_plants, method="bray", binary=FALSE, diag=TRUE) %>% as.matrix()
BC_res_par_mantel_common_plants <- mantel(BC_par_res_common_plants, Distance , method="spear"); BC_res_par_mantel
...
```{r}
# Chao-Sorensen index for parasitoids
CS_par_res_common_plants<- dis.chao(para_pivot2_common_plants, index="sorensen", version="rare", freq=NULL) %>%  as.matrix()
...
# Chao-Sorensen index for parasitoids
CS_par_res_rare_common_plants<- dis.chao(para_pivot2_rare_common_plants, index="sorensen", version="rare", freq=NULL) %>%  as.matrix()
CS_par_res_mantel_rare_common_plants <- mantel(CS_par_res_rare_common_plants, Distance , method="spear"); CS_par_res_mantel_common_plants
...
para_pivot2_common_common_plants[is.na(para_pivot2_common_common_plants)] <- 0 # Replacing NA values with 0
# Chao-Sorensen index for parasitoids
CS_par_res_common_common_plants<- dis.chao(para_pivot2_common_common_plants, index="sorensen", version="rare", freq=NULL) %>%  as.matrix()
CS_par_res_mantel_common_common_plants <- mantel(CS_par_res_common_common_plants, Distance , method="spear"); CS_par_res_mantel
...
# data for parasitoids
para_pivot_rare_common_plants<-  MASTER  %>%
  filter(`guild` == "PAR",rarity =="1",common_plants_only =="1" ) %>% # pick only parasitoids (PAR) 
  select(locality, PAR_sp)
...
sor_mantel_par_rare_common_plants <- mantel(sorensen_par_rare_common_plants, Distance, method="spear"); sor_mantel_par_rare_common_plants #Mantel test for parasitoids
...
# data for parasitoids
para_pivot_common_plants<-  MASTER  %>%
  filter(`guild` == "PAR",rarity =="0",common_plants_only =="1" ) %>% # pick only parasitoids (PAR) 
  select(locality, PAR_sp)
...
BC_FIG_Sy <-ggplot(all_plants_BC, aes(x = distance, y = hodnota, 
  ...
  scale_color_manual(values = c("All" = "darkgreen", 
                                "Common" = "brown", 
                                "Rare" = "blue")) +
...
Figure_SX_Rarity <- ggarrange(Sor_FIG_Sy, CS_FIG_Sy, BC_FIG_Sy, Sor_FIG_Sy2, CS_FIG_Sy2, BC_FIG_Sy2, common.legend = TRUE, legend = "bottom", nrow = 2, ncol = 3) ;Figure_SX_Rarity 
